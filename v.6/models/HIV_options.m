...
%Dynamics of HIV infection of CD4+ T cells. Mathematical biosciences, 114(1), 81-125.

% === PARAMETER CONFIGURATION ===
model.param_values =[10, 0.02, 2.4e-5, 0.24, 1000, 2.4];
model.param_names = {'lambda', 'rho', 'eta', 'delta', 'N', 'c'};

% === STATE CONFIGURATION ===
model.state_names = {'T', 'T*', 'V'};
model.x0 = [1000, 0, 1e-3];

% === STATE TO ANALYZE ===
model.state_to_analyze = 'V';
...
model.param_range_min    = [5,  0.01, 1e-6, 0.2, 600, 0.24];
model.param_range_max    = [20,  0.08, 1e-4, 0.7, 3000, 5];
model.param_range_step   = [0.25, 0.001, 1e-6, 0.01, 50, 0.05]; % used only in 'custom_step'
model.param_range_npoints = 200;   % used only in 'default_orders' and 'minmax_npoints'

% === INITIAL CONDITIONS RANGE VALUES ===
opts.IC_to_analyze = 'T';
% Available modes:
